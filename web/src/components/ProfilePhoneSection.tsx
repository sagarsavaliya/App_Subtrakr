 ...
 *  "Unverified" phone state to handle, unlike email. */
export function ProfilePhoneSection({ initialPhone }: Props) {
  ...
  return (
    <div className="glass flex items-center justify-between rounded-2xl p-4">
      <div>
        <p className="text-sm font-medium">{displayPhone ?? "No mobile number on file"}</p>
        {displayPhone && (
          <span className="mt-1 inline-block rounded-full bg-glow/15 px-2 py-0.5 text-[11px] font-semibold text-glow">
            Verified
          </span>
        )}
      </div>
      <button
        ...
        className="glass rounded-full px-4 py-1.5 text-xs font-semibold text-ink hover:border-glow/30"
      >
